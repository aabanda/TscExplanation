...
db = str(sys.argv[1])
transformation = str(sys.argv[2])
warp_level = float(sys.argv[3])
...
for repetition in range(0, 10):
    ...
    for ind in range(len(test_y)):
        ...
        w = np.loadtxt('neig/%s/%s/%s/weights_%d.txt' % (db, transformation, classifier, ind))
        ...
        if np.sum(w)==0 :
            ...
            for type2 in np.array([10,50,90]):
                ...
                if (int_to_warp[1] - int_to_warp[0]) < np.round(0.3 * len(ref)).astype(int):
                    add = np.round(
                        (np.round(0.3 * len(ref)).astype(int) - (int_to_warp[1] - int_to_warp[0])) / 2).astype(int)
                    if int_to_warp[0] - add < 1:
                        int_to_warp[1] = int_to_warp[1] + 2 * add
                    elif int_to_warp[1] + add > len(ref):
                        int_to_warp[0] = int_to_warp[0] - add
...
# a=np.array([0.62,0.61,0.75])
# b=np.array([0.42,0.40,0.56])
